...
# Production Stage Parameter Tracking Model
class ProductionStageParameters(db.Model):
    """Track required parameters at the start and end of each production stage"""
    __tablename__ = 'production_stage_parameters'
    
    id = db.Column(db.Integer, primary_key=True)
    job_id = db.Column(db.Integer, db.ForeignKey('production_job_new.id'), nullable=False)
    stage = db.Column(db.String(50), nullable=False)  # transfer, cleaning_24h, cleaning_12h, grinding, packing
    parameter_type = db.Column(db.String(20), nullable=False)  # 'start' or 'end'
    
    # Common parameters for all stages
    timestamp = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    operator_name = db.Column(db.String(100), nullable=False)
    machine_name = db.Column(db.String(100))
    temperature = db.Column(db.Float)  # Environmental temperature
    humidity = db.Column(db.Float)  # Environmental humidity
    notes = db.Column(db.Text)
    
    # Weight/Quantity tracking
    input_weight_kg = db.Column(db.Float)
    output_weight_kg = db.Column(db.Float)
    moisture_content_start = db.Column(db.Float)  # Moisture % at start
    moisture_content_end = db.Column(db.Float)    # Moisture % at end
    
    # Quality parameters
    foreign_matter_percentage = db.Column(db.Float)
    damaged_grains_percentage = db.Column(db.Float)
    broken_grains_percentage = db.Column(db.Float)
    test_weight_kg_per_hl = db.Column(db.Float)  # Test weight per hectolitre
    
    # Machine-specific parameters
    machine_speed_rpm = db.Column(db.Float)  # For grinding machines
    screen_size_mm = db.Column(db.Float)     # Screen size used
    pressure_bar = db.Column(db.Float)       # Operating pressure
    
    # Process-specific parameters
    cleaning_duration_hours = db.Column(db.Float)  # For cleaning stages
    bin_number = db.Column(db.String(50))           # Bin/container used
    batch_number = db.Column(db.String(50))         # Batch identifier
    
    # Photos for documentation
    process_photo = db.Column(db.String(255))       # Process photo
    sample_photo = db.Column(db.String(255))        # Sample photo
    ...
    def __repr__(self):
        return f'<ProductionStageParameters {self.stage}-{self.parameter_type} Job:{self.job_id}>'