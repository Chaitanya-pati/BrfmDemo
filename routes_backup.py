...
def production_execution():
    if request.method == 'POST':
        try:
            ...
            if action == 'start_job':
                ...
                job.operator_name = request.form['operator_name']
    ...
    active_jobs = ProductionJob.query.filter(ProductionJob.status.in_(['pending', 'in_progress'])).all()
    approved_plans = ProductionPlan.query.filter_by(status='approved').all()
    transfer_jobs = ProductionJob.query.filter_by(stage='transfer').all()
...
def reports():
    ...
    active_jobs = ProductionJob.query.filter_by(status='in_progress').count()
